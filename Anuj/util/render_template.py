...
async def render_page(id, secure_hash, src=None):
    file = await AnujBot.get_messages(int(LOG_CHANNEL), int(id))
    file_data = await get_file_ids(AnujBot, int(LOG_CHANNEL), int(id))
    ...
    if tag in ["video", "audio"]:
        template_file = "Anuj/template/req.html"
    else:
        template_file = "Anuj/template/dl.html"
